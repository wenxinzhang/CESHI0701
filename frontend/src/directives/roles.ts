...
function checkRolePermission(el: HTMLElement, binding: RolesBinding): void {
  ...
  // 将角色对象数组转换为角色名称数组
  const roleNames = userRoles.map((ur: { role: { id: number; name: string; label: string | null } }) => ur.role.name)

  // 确保指令值为数组格式
  const requiredRoles = Array.isArray(binding.value) ? binding.value : [binding.value]

  // 检查用户是否具有所需角色之一
  const hasPermission = requiredRoles.some((role: string) => roleNames.includes(role))
