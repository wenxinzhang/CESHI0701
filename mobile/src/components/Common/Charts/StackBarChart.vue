<!--
  组件名称：StackBarChart - 堆叠柱状图组件

  功能描述：
    基于 ECharts 封装的堆叠柱状图组件，支持多个数据系列堆叠展示、响应式尺寸、暗黑模式等功能

  使用方式：
    <StackBarChart
      :seriesData="[
        { name: '直接访问', data: [320, 302, 301, 334, 390, 330] },
        { name: '邮件营销', data: [120, 132, 101, 134, 90, 230] },
        { name: '联盟广告', data: [220, 182, 191, 234, 290, 330] }
      ]"
      :xAxisData="['周一', '周二', '周三', '周四', '周五', '周六']"
 ...
 * @property {string[]} [colors=[]] - 柱状图颜色数组
...
const updateChart = () => {
  ...
  const series = props.seriesData.map((item, index) => ({
    ...
    itemStyle: {
      ...
      borderRadius: index === props.seriesData.length - 1 ? [4, 4, 0, 0] : [0, 0, 0, 0]
    },
    emphasis: {
      focus: 'series'
    }
...
// 监听数据变化
watch(
  () => [props.seriesData, props.xAxisData, props.legend, props.title, props.colors, props.theme],
  () => {
    nextTick(() => {
      updateChart()
    })
